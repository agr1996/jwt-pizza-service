...
class Metrics {
  ...
  sendMetricToGrafana(metrics) {
    const metricsArray = metrics.split('\n').map(metric => {
      const [prefix, values] = metric.split(' ');
      const [metricPrefix, source] = prefix.split(',');
      const [metricName, metricValue] = values.split('=');
      return {
        metricPrefix,
    ...
    //console.log('Payload being sent to Grafana:', JSON.stringify(payload));
    ...
    fetch(`${config.metrics.url}`, {
      ...
      if (!response.ok) {
        response.text().then(text => console.error('Error response from Grafana:', text));
      }
    }).catch((error) => {
